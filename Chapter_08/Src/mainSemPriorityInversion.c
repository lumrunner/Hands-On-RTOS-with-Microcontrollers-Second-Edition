...
void TaskA( void* argument )
{
    ...
    while(1)
    {
        ...
        if(xSemaphoreTake(semPtr, 200/portTICK_PERIOD_MS) == pdPASS)
        {
            ...
            SEGGER_SYSVIEW_PrintfHost("received semPtr: %lu \r\n", receivedCounter);
            blinkTwice(&GreenLed);
            xSemaphoreGive(semPtr);
        }
        else
        {
            ...
            timedoutCounter++;
            SEGGER_SYSVIEW_PrintfHost("FAILED to receive semPtr in time: %lu \r\n", timedoutCounter);
            RedLed.On();
        }
...
void TaskB( void* argument )
{
    ...
    {
        counter++;
        SEGGER_SYSVIEW_PrintfHost("starting iteration %lu ", counter);
